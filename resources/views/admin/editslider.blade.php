            ...
            <div class="card-body">
              ...
              @if (Session::has('status'))
               ...
               @endif
                ...
                {!!Form::open(['action'=>'App\Http\Controllers\SliderController@modifierslider',
                  'method'=>'POST','class'=>'cmxform','id'=>'commentForm','enctype'=>'multipart/form-data'])!!}
                 {{ csrf_field() }}
                 {{ Form::hidden ('id',$slider->id) }}
                  <div class="form-group">
                   {{Form::label('','description one',['for'=>'cname'])}}
                   {{Form::text('description_one',$slider->description_one,['class'=>'form-control','id'=>'cname'])}}
                  </div>
                  <div class="form-group">
                    {{Form::label('','description two',['for'=>'cname'])}}
                    {{Form::text('description_two',$slider->description_two,['class'=>'form-control','id'=>'cname'])}}
                   </div>
                   <div class="form-group">
